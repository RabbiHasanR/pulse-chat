...
# --- LEARNING: Page-based Pagination ---
# Friendly wrapper over offset. Accepts `page` + `page_size` instead of raw offset.
# Identical to offset at the DB level: offset = (page - 1) * page_size.
# Adds total_pages to the response, which requires a COUNT(*) — expensive on large tables.
class UserPageListView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        try:
            ...
            return error_response("page and page_size must be integers", status=400)

        queryset = ChatUser.objects.exclude(id=request.user.id).order_by('full_name')

        # COUNT(*) needed for total_pages — this is what makes page-based expensive
        ...
        return success_response(
            message="Users retrieved (page-based pagination)",
            data={
...
# --- LEARNING: Keyset Pagination ---
# Filters instead of skips. Uses the last seen `id` as a WHERE clause.
# No COUNT(*), no row scanning — the index jumps directly to `before_id`.
# Limitation: no random page access, sort must match the keyset column.
class ContactKeysetListView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        queryset = Contact.objects.filter(
            owner=request.user
        ).select_related('contact_user').order_by('-id')

        if before_id:
            try:
                # Keyset filter: WHERE id < before_id — index seek, no scanning
                queryset = queryset.filter(id__lt=int(before_id))
            except (ValueError, TypeError):
                return error_response("before_id must be an integer", status=400)

        # Fetch limit+1 to determine has_next without a COUNT(*)
        contacts = list(queryset[: limit + 1])
        has_next = len(contacts) > limit
        if has_next:
            contacts = contacts[:limit]

        next_before_id = contacts[-1].id if has_next else None

        serializer = ContactSerializer(contacts, many=True, context={'online_status_map': {}})

        return success_response(
            message="Contacts retrieved (keyset pagination)",
            data={
                "results": serializer.data,
                "pagination": {
                    "limit": limit,
